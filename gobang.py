...
class Gobang(game.Game):
    ...
    def AI_player(self):
        ...
        boundary = -1                                                    #有棋子的范围，-1表示棋盘为空
        for i in range(8):
            flag = False                                                 #该范围有棋子 = True，初始化认为没有棋子
            for x in range(7 - i, 7 + i + 1):
                if self.board[x][7 - i] != EMPTY or self.board[x][7 + i] != EMPTY:
                    flag = True
                    break
            if flag == False:                                             #没有棋子则横向搜索
                for y in range(7 - i, 7 + i + 1):
                    if self.board[7 - i][y] != EMPTY or self.board[7 + i][y] != EMPTY:
                        flag = True
                        break
            if flag == False:
                break
        ...
        x, y, alpha = Search(self.board, boundary, EMPTY, BLACK, WHITE, self.black)
